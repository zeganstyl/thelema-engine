...
void main() {
    ...
    vec3 normal = texture2D(normalTex, vUv * 3.0 +
      0.8*vec2(
        cos(time*0.1),
        sin(time*0.1)
      ) +
      0.001*vec2(
        cos(1.7 + time*1.2+3.2*100.0*vUv.x),
        sin(1.7 + time*1.0+3.0*100.0*vUv.y)
      )
      ).rgb * 0.5 - 0.5;
    ...
    gl_FragColor.a = 0.7;
    gl_FragColor.rgb = mix(basecolor, textureCube(envTex, dir).rgb, 0.3);
}
""")
