...
open class GestureDetector(
    ...
    fun touchUp(x: Float, y: Float, pointer: Int, button: Int): Boolean {
        ...
        if (inTapRectangle) { // handle taps
            if (lastTapButton != button || lastTapPointer != pointer || APP.time - lastTapTime > tapCountInterval || !isWithinTapRectangle(x, y, lastTapX, lastTapY)) tapCount = 0
            tapCount++
            lastTapTime = APP.time
            lastTapX = x
            lastTapY = y
    ...
    fun isLongPressed(duration: Float): Boolean {
        return if (gestureStartTime == 0L) false else APP.time - gestureStartTime > (duration * 1000000000L).toLong()
    }
    ...
    internal class VelocityTracker {
        ...
        private fun getAverage(values: FloatArray, numSamples: Int): Float {
            var numSamples1 = numSamples
            numSamples1 = min(sampleSize, numSamples1)
            var sum = 0f
            for (i in 0 until numSamples1) {
                sum += values[i]
            }
            return sum / numSamples1
        }
        ...
        private fun getAverage(values: LongArray, numSamples: Int): Long {
            ...
            return if (numSamples1 == 0) 0 else sum / numSamples1
        }
        ...
        private fun getSum(values: FloatArray, numSamples: Int): Float {
            ...
            return if (numSamples1 == 0) 0f else sum
        }
    }
