...
class CatmullRomSpline(var controlPoints: List<IVec>, var continuous: Boolean = false) {
    ...
    private val tmp2: IVec = controlPoints[0].copy()
    private val tmp3: IVec = controlPoints[0].copy()
    ...
    fun nearest(`in`: IVec?, start: Int = 0, count: Int = spanCount): Int {
        var start1 = start
        while (start1 < 0) start1 += spanCount
        var result = start1 % spanCount
        var dst = `in`!!.dst2(controlPoints[result])
        for (i in 1 until count) {
            val idx = (start1 + i) % spanCount
