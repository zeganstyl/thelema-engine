...
class Frustum() {
    constructor(inverseProjectionView: IMat4): this() {
        setFromMatrix(inverseProjectionView)
    }
     ...
     */
    fun setFromMatrix(inverseViewProjection: IMat4) {
    ...
    fun isPointInFrustum(point: IVec3): Boolean {
        for (i in planes.indices) {
            val result = planes[i].distance(point)
    ...
    fun isPointInFrustum(x: Float, y: Float, z: Float): Boolean {
        for (i in planes.indices) {
            val result = planes[i].distance(x, y, z)
            if (result < 0f) return false
        }
        return true
    }
    ...
    fun minMaxInFrustum(minX: Float, minY: Float, minZ: Float, maxX: Float, maxY: Float, maxZ: Float): Boolean {
        var i = 0
        val len2 = planes.size
        while (i < len2) {
            if (planes[i].distance(maxX, maxY, maxZ) >= 0f) {
                i++
                continue
            }
            if (planes[i].distance(maxX, maxY, minZ) >= 0f) {
                i++
                continue
            }
            if (planes[i].distance(maxX, minY, maxZ) >= 0f) {
                i++
                continue
            }
            if (planes[i].distance(maxX, minY, minZ) >= 0f) {
                i++
                continue
            }
            if (planes[i].distance(minX, maxY, maxZ) >= 0f) {
                i++
                continue
            }
            if (planes[i].distance(minX, maxY, minZ) >= 0f) {
                i++
                continue
            }
            if (planes[i].distance(minX, minY, maxZ) >= 0f) {
                i++
                continue
            }
            if (planes[i].distance(minX, minY, minZ) >= 0f) {
                i++
                continue
            }
            return false
    ...
    fun boundsInFrustum(x: Float, y: Float, z: Float, halfWidth: Float, halfHeight: Float, halfDepth: Float): Boolean {
        ...
        val len2 = planes.size
        while (i < len2) {
            if (planes[i].distance(x + halfWidth, y + halfHeight, z + halfDepth) >= 0f) {
                i++
                continue
            }
            if (planes[i].distance(x + halfWidth, y + halfHeight, z - halfDepth) >= 0f) {
