...
interface IComponentDescriptor {
    ...
    fun forEachDescriptorInBranch(block: (descriptor: IComponentDescriptor) -> Unit) {
        block(this)
        descriptors.iterate {
            it.forEachDescriptorInBranch(block)
        }
    }
}

/** @param I interface type */
inline fun <reified I: IEntityComponent, reified New: IEntityComponent> IComponentDescriptor.replaceDescriptor(noinline create: () -> New) {
    replaceDescriptor(I::class.simpleName!!, descriptor(New::class.simpleName!!, create))
}

@Suppress("UNCHECKED_CAST")
inline fun <reified New: IEntityComponent> IComponentDescriptor.replaceDescriptor(
    name: String,
    noinline create: () -> New,
