...
open class OrbitCameraControl(
    ...
    /** If equals -1, then any button will have dragging effect */
    var draggingButton: Int = if (APP.isMobile) MOUSE.LEFT else MOUSE.MIDDLE
    ...
    var targetDistance: Float = targetDistance
        set(value) {
            field = value
    ...
    open fun update(delta: Float = APP.deltaTime) {
        if (isEnabled) {
    ...
    open fun buttonDown(x: Float, y: Float, button: Int) {
        if (isEnabled) {
            ...
            if (button == draggingButton || draggingButton == -1) {
                startX = x
                startY = y
            }
        }
    ...
    open fun dragged(x: Float, y: Float) {
        if (isEnabled) {
            if (pressedButton == draggingButton || draggingButton == -1) {
                if (KB.shift && isTranslationEnabled) {
                    // translate
