...
class PrincipledBSDF(): ShaderNode() {
    constructor(block: PrincipledBSDF.() -> Unit): this() { block(this) }
    ...
    var maxNumPointLights: Int = 10
    var maxNumSpotLights: Int = 1
    ...
    override fun readJson(json: IJsonObject) {
        ...
        maxNumDirectionLights = json.int("maxNumDirectionLights", 1)
        maxNumPointLights = json.int("maxNumPointLights", 1)
        maxNumSpotLights = json.int("maxNumSpotLights", 1)
            ...
            for (i in lights.indices) {
                ...
                if (light.isLightEnabled) {
                    when (light.lightType) {
                        ...
                        LightType.Point -> {
                            light as PointLight
                            light.node.getGlobalPosition(tmpVec)
                            shader["uPointLightPosition[$pointLightIndex]"] = tmpVec
...
uniform vec3 uSpotLightDirection[$maxNumSpotLights];
