 ...
 */
class Stage(
    var viewport: Viewport = ScreenViewport(),
    val batch: Batch = SpriteBatch()
) : IKeyListener, IMouseListener {
    constructor(
        viewport: Viewport = ScreenViewport(),
        batch: Batch = SpriteBatch(),
        block: Stage.() -> Unit
    ): this(viewport, batch) { block(this) }
    ...
    init {
        root.stage = this
        viewport.camera.useViewMatrix = false

        viewport.update(APP.width, APP.height, true)
    }
    ...
    fun update(delta: Float = APP.deltaTime) { // Update over actors. Done in act() because actors may change position, which can fire enter/exit without an input event.
        ...
        val type = APP.platformType
        if (type == APP.Desktop || type == APP.WebGL) mouseOverActor = fireEnterAndExit(mouseOverActor, mouseScreenX, mouseScreenY, -1)
        root.act(delta)
    }
...
//            }
     ...
     */
    fun toScreenCoordinates(coords: IVec2, transformMatrix: IMat4): IVec2 {
    ...
    fun calculateScissors(
        localRectX: Float,
        localRectY: Float,
        localRectWidth: Float,
        localRectHeight: Float,
        scissorRect: IRectangle
    ) {
        val viewportX = viewport.screenX.toFloat()
        val viewportY = viewport.screenY.toFloat()
        val viewportWidth = viewport.screenWidth.toFloat()
        val viewportHeight = viewport.screenHeight.toFloat()
