...
//  - if the user did not move the wheel for more than 250ms
            ...
            mouse.listeners.forEach { it.scrolled(scrollAmount) }
    ...
    private val cursorPosCallback = staticCFunction {
        ...
        if (mouse.mousePressed > 0) {
            mouse.listeners.forEach { it.dragged(mouse.mouseX, mouse.mouseY, 0) }
        } else {
            mouse.listeners.forEach { it.moved(mouse.mouseX, mouse.mouseY) }
        }
    }
    private val mouseButtonCallback = staticCFunction {
        ...
        if (action == GLFW_PRESS) {
            mouse.mousePressed++
            mouse.listeners.forEach { it.buttonDown(convButton, mouse.mouseX, mouse.mouseY, 0) }
        } else {
            mouse.mousePressed = max(0, mouse.mousePressed - 1)
            mouse.listeners.forEach { it.buttonUp(convButton, mouse.mouseX, mouse.mouseY, 0) }
        }
    }
