...
import org.ksdfv.thelema.gl.GL_SHADING_LANGUAGE_VERSION
import org.ksdfv.thelema.gl.GL_VERSION
...
class GLFWGL: AbstractGL() {
    ...
    override fun glBufferData(target: Int, size: Int, data: IByteData?, usage: Int) {
        glfw.glBufferData(target.uint(), (data?.size ?: 0).toLong(), data?.ptr(), usage.uint())
    }
    ...
    override fun glCompressedTexImage2D(target: Int, level: Int, internalformat: Int, width: Int, height: Int, border: Int,
        ...
        glfw.glCompressedTexImage2D(target.uint(), level, internalformat.uint(), width, height, border, data.size, data.ptr())
    }
    ...
    override fun glDeleteBuffer(buffer: Int) {
        glfw.glDeleteBuffers(1, buffer.uintPtr())
    }
    ...
    override fun glDeleteTexture(texture: Int) {
        glfw.glDeleteTextures(1, texture.uintPtr())
    }
    ...
    override fun glGenBuffer(): Int {
        return uintOut { glfw.glGenBuffers(1, it) }.sint()
    }
    ...
    override fun glGenTexture(): Int {
        return uintOut { glfw.glGenTextures(1, it) }.sint()
    }
    ...
    override fun glShaderBinary(n: Int, shaders: IntArray, binaryformat: Int, binary: IByteData, length: Int) {
        glfw.glShaderBinary(n, shaders.uintPtr(), binaryformat.uint(), binary.ptr(), binary.size)
    }
    ...
    override fun glDeleteFramebuffer(framebuffer: Int) {
        glfw.glDeleteFramebuffers(1, framebuffer.uintPtr())
    }
    ...
    override fun glDeleteRenderbuffer(renderbuffer: Int) {
        glfw.glDeleteRenderbuffers(1, renderbuffer.uintPtr())
    }
    ...
    override fun glGenFramebuffer(): Int {
        return uintOut { glfw.glGenFramebuffers(1, it) }.sint()
    }

    override fun glGenRenderbuffer(): Int {
        return uintOut { glfw.glGenRenderbuffers(1, it) }.sint()
    }
