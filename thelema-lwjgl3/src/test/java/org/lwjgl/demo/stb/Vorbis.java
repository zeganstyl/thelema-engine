...
public final class Vorbis implements AutoCloseable {
    ...
    public static ByteBuffer ioResourceToByteBuffer(String resource, int bufferSize) throws IOException {
        ...
        if (Files.isReadable(path)) {
            ...
            try (
                    InputStream source = Vorbis.class.getClassLoader().getResourceAsStream(resource);
    ...
    private Vorbis(String filePath) {
        ...
        glfwSetFramebufferSizeCallback(window, (window, width, height) -> {
            ...
            this.clientW = Math.round(width / scaleX);
            this.clientH = Math.round(height / scaleY);
        });
    ...
    private static class VorbisTrack implements AutoCloseable {
        ...
        VorbisTrack(String filePath, AtomicInteger sampleIndex) {
            ...
            System.out.println(samplesLength);
        ...
        void skip(int direction) {
            seek(Math.min(Math.max(0, stb_vorbis_get_sample_offset(handle) + direction * sampleRate), samplesLength));
        }
    ...
    private static class GraphicsRenderer implements AutoCloseable {
        ...
        void render(
            ...
            // Progress text
            int minutes = (int)Math.floor(progressTime / 60.0);
            int seconds = (int)Math.floor(progressTime - minutes * 60.0);
            int millis  = (int)Math.floor((progressTime - minutes * 60.0 - seconds) * 100.0);
