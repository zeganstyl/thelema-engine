...
interface IImg {
    fun createImage(): IImage =
        Image(0, 0, DATA.nullBuffer)

    fun load(
        url: String,
        out: IImage = createImage(),
        sourceLocation: Int = FileLocation.Internal,
        response: (status: Int, img: IImage) -> Unit = { _, _->}
    ): IImage =
        load(FS.file(url, sourceLocation), out, response)

    fun load(file: IFile, out: IImage = createImage(), response: (status: Int, img: IImage) -> Unit = { _, _->}): IImage {
        out.name = file.path
    ...
    fun load(byteArray: ByteArray, out: IImage = createImage(), response: (status: Int, img: IImage) -> Unit = { _, _->}): IImage {
    ...
    fun load(data: IByteData, out: IImage = createImage(), response: (status: Int, img: IImage) -> Unit = { _, _->}): IImage
}