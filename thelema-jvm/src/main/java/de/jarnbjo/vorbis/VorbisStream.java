...
public class VorbisStream {
   ...
   private LinkedList<AudioPacket> audioPackets = new LinkedList<>();
   ...
   public int readPcm(ByteBuffer buffer, int offset, int length) throws IOException {
      synchronized (streamLock) {
         ...
         for(i=currentPcmIndex; i<currentPcmLimit && arrIx<length; i++) {
            buffer.put(currentPcm[i]);
            arrIx++;
            //buffer.put(offset+arrIx++, currentPcm[i]);
            written++;
         }
         currentPcmIndex=i;
         return written;
