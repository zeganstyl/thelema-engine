...
class AudioPacket {
   ...
   private final int n; // block size
   ...
   protected AudioPacket(final VorbisStream vorbis, final BitInputStream source) throws IOException {
      ...
      int modeNumber = source.getInt(Util.ilog(modes.length - 1));
      ...
      Mapping mapping = mappings[mode.getMapping()];
      ...
      Floor[] channelFloors = new Floor[channels];
      boolean[] noResidues = new boolean[channels];
